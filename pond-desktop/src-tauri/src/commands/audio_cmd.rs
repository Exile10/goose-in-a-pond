...
pub async fn run_voice_pipeline(
    ...
    if pipeline_flag.0
        ...
        .is_err()
    {
        tracing::debug!("run_voice_pipeline: duplicate invocation dropped (pipeline already active)");
        return Ok(());
    }
    ...
    if let Some((farewell, is_exit)) = tts_text::check_dismissal(&transcript) {
        ...
        match fetch_tts_bytes(&client, &base_url, farewell).await {
            Ok(bytes) => { let _ = play_wav_bytes_interruptible(bytes, kill_flag.clone(), audio_handle.clone()).await; }
    ...
    let tts_task   = tokio::spawn(async move {
        ...
        while let Some(text) = tts_rx.recv().await {
            ...
            match fetch_tts_bytes(&tts_client, &tts_url, &text).await {
                Ok(bytes) => { let _ = play_wav_bytes_interruptible(bytes, tts_kill.clone(), tts_audio.clone()).await; }
