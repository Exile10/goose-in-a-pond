...
pub struct TurnMetrics {
    ...
    /// Tokens the model spent reasoning, when anybody counted.
    ///
    /// `None` is not zero: it means unmeasured, which is every row written
    /// before migration 0008 and every provider that reports no ProviderStats.
    /// Folding those into a zero would average evidence that does not exist
    /// into any conclusion drawn from this table.
