...
async fn update_settings(
    ...
    // "enable Matter first" error impossible to act on. Sent on every save, not
    // only when the keys are in the patch, so a Retry from the Devices tab
    // reconnects; the runtime is a no-op when it is already in this state.
