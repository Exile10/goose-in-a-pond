...
| 7 | **Personal context streaming** — on-pond, on-mobile, and internet accounts | [PAI-8](./08-personal-context-streaming.md) | DESIGNED. **The only one of the eight with no landed code**, and the reason is a two-way dependency rather than a queue: PAI-2 P6b's third part (P3's redaction chokepoint) has no call site to wire until PAI-8 creates one, and PAI-8's P8 needs a proposer — which PAI-7 P3 now has, and PAI-7 P4's reviewer now runs, so that edge has fallen. The remaining decision is which of P6b-part-3 and PAI-8 P1 goes first, and it is a decision, not more work to schedule |
...
**PAI-6 and PAI-7 are both COMPLETE as of 2026-08-11 — P1 through P8 each.** So **seven of the
eight workstreams now have landed code**, and **PAI-8 is the only one that is still design only**.
PAI-1 is complete and gained P9; PAI-2 runs through P8a; PAI-3, PAI-4, PAI-5, PAI-6 and PAI-7 are
landed.
...
**What is left is one decision and two measurements, not a queue.** The decision is PAI-2 P6b's
remaining two parts against PAI-8: the draft gate for outbound connector actions, and P3's third
redaction chokepoint, which has *no call site to wire until PAI-8 creates one*. That is circular, and
somebody has to choose which side moves first. The measurements are PAI-7's — a completed proactive
