...
| [08](./pai/08-personal-context-streaming.md) | Personal context streaming | Personal context streaming | 01, 02, 07 | DESIGNED — **the only workstream with no landed code**, and now the only thing standing between the programme and eight-for-eight. Its P8 was blocked on PAI-7 having a proposer; PAI-7 is complete and its reviewer runs one, so that edge has fallen. What remains is a **two-way** dependency with PAI-2 P6b, not a queue: P6b's third part is a redaction chokepoint with no call site to wire until PAI-8 creates one. Which side moves first is a decision |
