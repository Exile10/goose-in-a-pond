...
| llama.cpp on-disk preamble snapshot | The retained KV cache makes the SECOND turn of a session cheap and does nothing for the first, so every cold start -- a new session, a model swap, a restart -- re-decodes a byte-identical preamble. Measured on an M-series host: 6,685 prompt tokens prefill in 24.1s at 277 tok/s, against 26-40 new sessions in a day, while the same KV state reads back from disk in a fraction of a second. `prompt_snapshot.rs` persists the **stable prefix** -- the longest run every prompt for a model has shared since the process started -- keyed by `(model path, len, mtime, n_ctx, layout fingerprint)` with the format version in the FILE NAME, so an incompatible snapshot is never opened rather than opened and rejected. Restored on the `CreateContext` path and written once per slot from a cache trimmed to exactly that prefix. Truncating before the write is the privacy property, not an optimisation: a KV blob encodes the tokens that produced it, and the shared prefix cannot contain anything a second prompt did not also contain -- GIAP's memories and turn context ride the user message, which diverges. Never shared BETWEEN models (KV state bakes in layer count, head dims and rope), which is the point: a model swap is the most expensive cold start there is and each keeps its own file. Loaded tokens are checked to be a genuine prefix of the prompt rather than trusted, because the key cannot see a changed system prompt or tool set; every failure path falls back to a normal prefill, holding the engine's standing rule that a bad cache never fails a turn. Uses only `state_seq_save_file`/`state_seq_load_file` already exposed by the pinned `llama-cpp-2 =0.1.146` -- no sys-crate change. **Verified on hardware** (gemma-4-E2B-it Q4_K_M, M-series, 2026-08-16): a 1,202-token snapshot is 21.2 MiB, i.e. **18.0 KiB/token** -- which independently reproduces the 18 KiB/token measured for E2B on the Orin, so the blob really is the KV cache. Restore-plus-tail-decode 132 ms against 1,898 ms for the equivalent full prefill (**14.4x**); the restored context predicts the same argmax and the same top five, with a max logit delta of 0.007 against logits spanning ~20 -- batch-shape float noise, not divergence. Bitwise equality is NOT asserted and demanding it was the first version's mistake: the two paths batch differently by construction and llama.cpp's reduction order follows the batch shape. A snapshot that does not prefix the incoming prompt is refused rather than restored, which is the shape a changed system prompt or tool set produces. Live test: `a_restored_snapshot_answers_identically_to_a_full_prefill`, `#[ignore]`d, run with `--ignored`. Upstreamable, and a natural extension of the KV cache patch above. | `crates/goose-local-inference/src/llamacpp/prompt_snapshot.rs` (new), `crates/goose-local-inference/src/llamacpp/inference_engine.rs`, `crates/goose-local-inference/src/llamacpp/mod.rs` |
